...
|---|---|---|---|
| `10000000-0000-0000-0000-000000000001` | `Олена Коваленко` | `olena.kovalenko@example.com` | `true` |
| `10000000-0000-0000-0000-000000000002` | `Богдан Шевченко` | `bohdan.shevchenko@example.com` | `true` |
| `10000000-0000-0000-0000-000000000003` | `Оксана Мельник` | `oksana.melnyk@example.com` | `false` |

`tasks`:

| `Id` | `Title` | `Status` | `CreatorId` | `AssigneeId` | `PlannedStartAt` | `DueAt` | `CompletedAt` |
|---|---|---|---|---|---|---|---|
| `20000000-0000-0000-0000-000000000001` | `Підготувати квартальний звіт з продажів` | `New` | Олена `…0001` | Богдан `…0002` | 2026-10-01 09:00:00Z | 2026-10-10 17:00:00Z | null |
| `20000000-0000-0000-0000-000000000002` | `Передзвонити ключовому клієнту` | `Completed` | Богдан `…0002` | Олена `…0001` | 2026-09-01 09:00:00Z | 2026-09-05 17:00:00Z | 2026-09-04 15:30:00Z |
| `20000000-0000-0000-0000-000000000003` | `Очистити дублікати контактів` | `Cancelled` | Олена `…0001` | Богдан `…0002` | null | null | null |

Checks: BR1 (only the Completed row has `CompletedAt`), BR2 (due ≥ planned or null), BR5 (creator ≠ assignee on
every row), BR3 (no task assigned to Оксана, the inactive employee), status values valid, e-mails lowercase and
unique. Оксана exists so BR3 can be tested against the database.
...
- `public sealed class PagedResult<T> : IReadOnlyList<T>`: holds `Items`, `NextCursor`, `HasNextPage`, and implements `IReadOnlyList<T>`.
- `public sealed record TaskListQuery(Guid AssigneeId, TaskItemStatus? Status = null, DateTimeOffset? DueFrom = null, DateTimeOffset? DueTo = null, Guid? CreatorId = null, int PageSize = TaskListQuery.DefaultPageSize, TaskCursor? Cursor = null);`
...
public Task<PagedResult<TaskItem>> ListTasksAsync(TaskListQuery query, CancellationToken cancellationToken = default)
...
**`ListTasksAsync`**
- `query` is null → `ArgumentNullException.ThrowIfNull(query)`.
- `query.Status` has a value that is not a defined member (`!Enum.IsDefined(query.Status.Value)`) →
  `new ArgumentOutOfRangeException(nameof(query), query.Status, "Unknown task status.")`.
- `effectivePageSize = Math.Clamp(query.PageSize, 1, 100)`.
- Keyset cursor:
  - If `query.Cursor.DueAt` has value: `.Where(t => t.DueAt > cursorDueAt || (t.DueAt == cursorDueAt && t.Id > cursorId) || t.DueAt == null)`.
  - If `query.Cursor.DueAt` is null: `.Where(t => t.DueAt == null && t.Id > cursorId)`.
- Query: `dbContext.Tasks.AsNoTracking().Where(t => t.AssigneeId == query.AssigneeId)`, then:
  - `.Where(t => t.Status == query.Status.Value)` if set;
  - `.Where(t => t.CreatorId == query.CreatorId.Value)` if set;
  - `.Where(t => t.DueAt >= query.DueFrom.Value)` if set;
  - `.Where(t => t.DueAt <= query.DueTo.Value)` if set;
  - `.OrderBy(t => t.DueAt).ThenBy(t => t.Id).Take(effectivePageSize + 1).ToListAsync(cancellationToken)`.
- If `items.Count > effectivePageSize`: remove last item, set `nextCursor = new TaskCursor(lastItem.DueAt, lastItem.Id)`.
- Returns `new PagedResult<TaskItem>(items, nextCursor)`.

**`ListTasksByAssigneeAsync`**
- Backward-compatible convenience wrapper that delegates directly to:
  `ListTasksAsync(new TaskListQuery(assigneeId, status), cancellationToken)`.
- Validates `status` parameter if provided (`ArgumentOutOfRangeException`).
