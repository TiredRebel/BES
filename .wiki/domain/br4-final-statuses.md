...
| Database | trigger `trg_tasks_br3_br4` on `UPDATE OF status` (planned, in `InitialCreate`) | a status change out of `Completed`/`Cancelled` → `23514` / `trg_tasks_br4_final_status`, for every writer including raw SQL ([ADR 0009](../../docs/adr/0009-br3-br4-enforced-by-trigger.md)) |
