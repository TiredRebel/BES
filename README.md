...
- **Мультиагентна взаємодія (Human-in-the-Loop):** Команда масштабується через ієрархічну AI-структуру:
  ...
  - **Specialized Workers:** автономні агенти, що працюють в ізольованих робочих просторах `git worktree` над чітко окресленими задачами (моделювання, тести, мутаційний аналіз).
...
| **3. Заземлення на AST (CodeGraph)** | Детерміністичне визначення символів і зв'язків замість векторних здогадок | У корені налаштовано `.mcp.json` із сервером `codegraph serve --mcp`, створено індекс `.codegraph/`. Усі типи, методи та простори імен ідеально відповідають синтаксичному дереву Roslyn. | ✅ Відповідає |
