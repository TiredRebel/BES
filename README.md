...
#### Доменний рівень
- TaskChangeType і сутність TaskHistoryEntry тримають аудит: хто, коли, який тип зміни, старе й нове значення.
- Reassign забороняє зміну виконавця для завдань у статусах Completed та Cancelled. Це прямий наслідок BR4: фінальний статус фіксує життєвий цикл цілком. Не можна заморозити статус і далі міняти власника завдання.
- Для активних завдань метод перевіряє BR3 (виконавець має бути активним) і BR5 (виконавець не може бути автором: `newAssignee.Id == CreatorId`). Порушення дає `BusinessRuleViolationException`. Тип той самий, що й при створенні, щоб контракт помилок не розходився між шляхами.
- Логіка перепризначення лишилась у домені свідомо, щоб інваріанти не розмазувались по сервісному шару.
- Валідація в TaskHistoryEntry локальна, всередині сутності, без окремих CheckRule і без універсального Guard на весь домен. Для аудитної сутності обрав простіший і читабельніший варіант, без зайвого ускладнення. За потреби піднімається до повнішого DDD.

#### Дані та міграції
- TaskHistoryEntryConfiguration описує таблицю task_history: зовнішні ключі на tasks і employees з поведінкою Restrict, щоб історія не втратила ні автора, ні завдання, плюс складений індекс (task_id, changed_at) під швидку хронологію.
- Оновив TaskManagementDbContext, створив міграцію AddReassignmentAndHistory.
- У міграції розширив тригер trg_tasks_br3_br4. Декларація тепер BEFORE INSERT OR UPDATE OF status, assignee_id ON tasks, тому перевірка активності виконавця (BR3) і блокування змін у фінальних статусах (BR4) діють і при зміні assignee_id. Спроба обійти домен прямим SQL упреться в trg_tasks_br4_final_status або trg_tasks_br3_assignee_active.

#### Сервісний шар
- ReassignTaskAsync знаходить завдання, актора й нового виконавця, викликає доменний Reassign і дописує запис у TaskHistory. BR3 на цьому шляху тримають домен і тригер, без дублюючої перевірки в сервісі, на відміну від CreateTaskAsync, де вона є, щоб при неактивному співробітнику, який є і автором, і виконавцем, повідомлялося саме BR3, а не BR5.
- Актор за замовчуванням в аудиті: перевантаження `ChangeTaskStatusAsync(taskId, newStatus, ct)` без `changedById` збережено для зворотної сумісності з початковим контрактом сервісу та існуючими тестами. У ньому як автор зміни фіксується поточний виконавець (`task.AssigneeId`) - це компроміс для неавторизованих викликів, де контекст користувача ще не передавався. Для явного аудиту додано перевантаження з `changedById`, де переданий ID обов'язково перевіряється на існування в базі (`Employees.AnyAsync`).
- Єдиний контракт помилок: у `ReassignTaskAsync` спрацювання тригерів БД (`trg_tasks_br3_assignee_active` та `trg_tasks_br4_final_status`) перехоплюється й транслюється в `BusinessRuleViolationException("BR3", ...)` та `BusinessRuleViolationException("BR4", ...)`. У `ChangeTaskStatusInternalAsync` перехоплюється `trg_tasks_br4_final_status` (перевірка BR3 у тригері прив'язана до зміни виконавця й на зміну статусу не спрацьовує). Голі інфраструктурні винятки `DbUpdateException`/`PostgresException` назовні не витікають: кожен метод перехоплює ті тригери, які здатні спрацювати саме на його шляху.
- Захист від брудного стану: у `ReassignTaskAsync` і `ChangeTaskStatusInternalAsync` блок `finally` при невдалому збереженні від'єднує `task` і `history` від контексту. Інакше наступний `SaveChanges` у тому ж контексті повторно відправив би вже відхилену зміну. `CreateTaskAsync` робить те саме з новим завданням.
- GetTaskHistoryAsync повертає хронологію змін завдання.
- Історія починається з першої зміни: CreateTaskAsync не створює запис у TaskHistory, оскільки початковий стан (автор, виконавець, час створення) уже зафіксований безпосередньо в таблиці tasks. Історія веде аудит мутацій, а не факт створення.

#### Тести
- Юніт-тести на Reassign: BR3, BR4, BR5 і незмінність виконавця для завершених завдань.
- Інтеграційні тести на Testcontainers: тригери, історія, обмеження, а також мапінг винятків БД на рівні сервісу (включно з перехопленням тригера BR4 при зміні статусу та перевіркою від'єднання сутностей).
- Сценарії й межові випадки прогнав, усі 120 тестів зелені (64 юніт + 56 інтеграційних). Покриття додав за цілісність поведінки, хоч ТЗ прямо цього й не вимагало.

> **Щодо AI:** використовував як прискорювач на бойлерплейті, це чернетки міграції й тестів, та для швидкої звірки вимог. Рішення по моделі, інваріантах Reassign, оновленню тригера й мапінгу винятків, а також фінальну перевірку схеми та тест-кейсів робив і валідував сам.
...
- **Якість та верифікація:** **120/120 тестів пройдено** (64 unit + 56 інтеграційних на реальному PostgreSQL 17 via Testcontainers; жодного оманливого In-Memory). Режим `TreatWarningsAsErrors` - 0 помилок, 0 попереджень.
...
[![Загальна високорівнева архітектура системи](docs/diagrams/system-context.visual-check.1440x900.light.png)](https://tiredrebel.github.io/BES/diagrams/system-context.html)
...
[![Модель сутностей та зв'язків](docs/diagrams/domain-model.visual-check.1440x900.light.png)](https://tiredrebel.github.io/BES/diagrams/domain-model.html)

> 🔍 **[Відкрити інтерактивну діаграму Archify (доменні сутності, інваріанти BR1-BR5)](https://tiredrebel.github.io/BES/diagrams/domain-model.html)**  
...
[![Пайплайн дворівневого захисту даних](docs/diagrams/defense-in-depth.visual-check.1440x900.light.png)](https://tiredrebel.github.io/BES/diagrams/defense-in-depth.html)

> 🔍 **[Відкрити інтерактивну діаграму Archify (C# Fail-Fast, PostgreSQL 17 інваріанти, OCC)](https://tiredrebel.github.io/BES/diagrams/defense-in-depth.html)**  
...
[![Agile Ітерація та інженерні фази](docs/diagrams/agile-iteration.visual-check.1440x900.light.png)](https://tiredrebel.github.io/BES/diagrams/agile-iteration.html)

> 🔍 **[Відкрити інтерактивну діаграму Archify (Agile фази, TDD, worktrees, аудит)](https://tiredrebel.github.io/BES/diagrams/agile-iteration.html)**  
