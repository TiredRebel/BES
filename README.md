...
- Для активних завдань метод перевіряє BR3 (виконавець має бути активним) і BR5 (виконавець не може бути автором). Порушення дає BusinessRuleViolationException. Тип той самий, що й при створенні, щоб контракт помилок не розходився між шляхами.
...
#### Сервісний шар
- ReassignTaskAsync знаходить завдання й нового активного виконавця, викликає доменний Reassign і дописує запис у TaskHistory.
- Єдиний контракт помилок. Коли спрацьовує тригер БД (trg_tasks_br3_assignee_active або trg_tasks_br4_final_status), сервіс ловить DbUpdateException чи PostgresException і транслює їх у BusinessRuleViolationException("BR3", ...) та BusinessRuleViolationException("BR4", ...). Голий інфраструктурний виняток назовні не протікає, тому контракт симетричний CreateTaskAsync.
- ChangeTaskStatusAsync тепер сам фіксує зміну статусу в історії.
...
- Інтеграційні тести на Testcontainers: тригери, історія, обмеження, а також мапінг винятків БД на рівні сервісу.
- Сценарії й межові випадки прогнав, усі 119 тестів зелені. Покриття додав за цілісність поведінки, хоч ТЗ прямо цього й не вимагало.
...
- **Якість та верифікація:** **119/119 тестів пройдено** (64 unit + 55 інтеграційних на реальному PostgreSQL 17 via Testcontainers; жодного оманливого In-Memory). Режим `TreatWarningsAsErrors` - 0 помилок, 0 попереджень.
