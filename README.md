...
#### Доменний рівень
- Додав `TaskChangeType` і сутність `TaskHistoryEntry` для аудиту.
- Метод `Reassign`: перевіряє стан завдання, бізнес-обмеження й оновлює `AssigneeId`.
- Логіка перепризначення свідомо на домені, щоб інваріанти не розмазувались по сервісному шару.
- Валідація в `TaskHistoryEntry` — локальні перевірки всередині сутності (без окремих `CheckRule` / універсального `Guard` на весь домен). Для цього завдання обрав простіший і читабельний варіант; за потреби легко піднімається до повнішого DDD.
#### Дані та міграції
- Додав і налаштував `TaskHistoryEntryConfiguration`, оновив `TaskManagementDbContext`, створено міграцію `AddReassignmentAndHistory`.
- У міграції оновив тригер `trg_tasks_br3_br4`, щоб перевірка активності та лімітів працювала також при зміні виконавця — інакше обмеження обходилися б через `Reassign`.
#### Сервісний шар
- У `TaskService`: `ReassignTaskAsync` (перевірка виконавця, ліміт активних завдань, перепризначення, запис в історію).
- `ChangeTaskStatusAsync` тепер також пише в історію.
- `GetTaskHistoryAsync` — отримання історії змін завдання.
#### Тести
- Юніт-тести на `Reassign`.
- Інтеграційні тести (Testcontainers) на тригери, історію та обмеження.
- Сценарії й edge cases перевірив і прогнав успішно; покриття додав для цілісності поведінки, навіть якщо ТЗ прямо цього не вимагало.
> **Щодо AI:** використовував як прискорювач на бойлерплейті (чернетки міграції/тестів) і для швидкої звірки вимог. Рішення по моделі, інваріантах `Reassign`, оновленню тригера та фінальну перевірку схеми й тест-кейсів робив і валідував самостійно.
